...
const Accounts = () => {
  ...
  // Memoized account mapping
  const accounts = useMemo(() => {
    return (globalAccounts || []).map((acc: any) => ({
      id: acc.accNo,
  ...
  // Memoized account groupings
  const { checkingAccounts, savingsAccounts, investmentAccounts, loanAccounts, totalBalance } = useMemo(() => {
    const checking = accounts.filter(account => account.type.includes('current') || account.type.includes('checking'));
    const savings = accounts.filter(account => account.type.includes('saving'));
    const investment = accounts.filter(account => account.type.includes('credit'));
    const loans = accounts.filter(account => account.type.includes('loan'));
  ...
  // Memoized raw accounts for transfer modal
  const rawAccounts = useMemo(() => globalAccounts, [globalAccounts]);
  ...
  const renderAccountCards = useCallback((accountsList: any[]) => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {accountsList.map((account, idx) => (
          <Card key={account.id || idx} className="h-auto border border-green-100 shadow-sm hover:shadow-lg transition-all duration-300">
            <CardContent className="p-5">
              <div className="flex items-start justify-between mb-4">
                ...
                {account.isMain && (
                  <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-[10px] font-bold uppercase tracking-wider">
                    Primary
                  </span>
                )}
              </div>
              ...
              <div className="mt-4 bg-gray-100 p-4 rounded-xl">
                ...
                <p className={`text-2xl font-bold tracking-tight ${account.balance < 0 ? 'text-red-600' : 'text-zinc-900'}`}>
                  {account.balance < 0 ? '-' : ''}{formatBalance(account.balance)}
                </p>
              </div>
              
              <div className="mt-5 flex gap-3">
                <Button size="sm" className="flex-1 rounded-lg bg-green-600 hover:bg-green-700 cursor-pointer" onClick={handleOpenTransferModal}>Transfer</Button>
              </div>
            </CardContent>
          </Card>
        ))}
        <Card 
          ...
          className="h-auto border-dashed border-2 hover:border-green-300 transition-colors flex items-center justify-center p-5 bg-gray-100 hover:bg-green-50/30 cursor-pointer group"
        >
          <div className="flex flex-col items-center justify-center py-6">
            ...
            <p className="font-semibold text-zinc-700 group-hover:text-green-700 transition-colors">Create New Account</p>
          </div>
        </Card>
      </div>
    );
  }, [handleOpenTransferModal, handleOpenAccountModal, formatBalance]);
      ...
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <Card key={i} className="border border-green-100 shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 3 }).map((_, i) => (
            <Card key={i} className="h-[220px] border border-green-100 shadow-sm">
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <Button className="gap-2 rounded-full bg-green-600 hover:bg-green-700 shadow-md hover:shadow-lg cursor-pointer" onClick={handleOpenAccountModal}>
      ...
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="mb-8 grid grid-cols-4 max-w-xl bg-green-50/50 p-1 border border-green-100 rounded-xl">
          <TabsTrigger value="all" className="rounded-lg cursor-pointer data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-green-700">All Accounts</TabsTrigger>
          <TabsTrigger value="savings" className="rounded-lg cursor-pointer data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-green-700">Savings</TabsTrigger>
          <TabsTrigger value="checking" className="rounded-lg cursor-pointer data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-green-700">Current</TabsTrigger>
          <TabsTrigger value="other" className="rounded-lg cursor-pointer data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-green-700">Credit Card</TabsTrigger>
        </TabsList>
