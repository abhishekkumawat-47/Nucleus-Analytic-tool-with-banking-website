        ...
        <CardContent className='p-6 bg-white'>
          <div className='flex flex-col sm:flex-row gap-4 mb-8 bg-gray-100/50 p-4 rounded-xl border border-gray-100'>
            ...
            <div className='flex flex-wrap gap-2'>
              ...
              {date && (
                <Button variant='outline' size='icon' className="rounded-lg cursor-pointer hover:bg-rose-50 hover:text-red-600 hover:border-rose-100 transition-colors" onClick={() => setDate(undefined)} title='Clear date filter'>
          ...
          <div className='space-y-4'>
            {error ? (
              <div className='text-center py-12 bg-rose-50/50 rounded-xl border border-rose-100'>
                ...
                <Button className='rounded-full cursor-pointer bg-red-600 hover:bg-rose-700' onClick={() => window.location.reload()}>
                  Refresh Page
                </Button>
              </div>
            ) : paginatedTransactions.length === 0 ? (
              <div className='text-center py-16 bg-gray-100/50 rounded-xl border border-dashed border-gray-200'>
                <div className="h-16 w-16 bg-green-100 rounded-full flex flex-col items-center justify-center mx-auto mb-4">
              ...
              paginatedTransactions.map((transaction) => {
                ...
                return (
                  <div key={transaction.id} className='flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-gray-100 hover:border-green-200 hover:shadow-sm transition-all group cursor-pointer bg-white'>
                    <div className='flex flex-col sm:flex-row sm:items-center gap-4 mb-2 sm:mb-0'>
                      <div className='flex-shrink-0 w-12 h-12 rounded-full bg-green-50 group-hover:bg-green-100 transition-colors flex items-center justify-center'>
                        {isSender ? (
                      ...
                      <div>
                        <h4 className='font-semibold text-zinc-900 group-hover:text-green-700 transition-colors'>
                    ...
                    <div className='flex flex-col sm:flex-row sm:items-center gap-4 mt-2 sm:mt-0'>
                      ...
                      <div className='flex flex-col sm:items-end min-w-[100px]'>
                        <span className={`font-bold text-lg tracking-tight ${isSender ? "text-red-600" : "text-emerald-600"}`}>
          ...
          {totalPages > 1 && (
            <Pagination className='mt-8 pt-4 border-t border-gray-100'>
              <PaginationContent>
                <PaginationItem>
                  <PaginationPrevious href='#' className="cursor-pointer hover:bg-green-50 hover:text-green-700 rounded-lg" onClick={(e) => { e.preventDefault(); setCurrentPage(Math.max(1, currentPage - 1)); }} />
                </PaginationItem>
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  const pageNumber = i + 1;
                  return (
                    <PaginationItem key={pageNumber}>
                      <PaginationLink href='#' isActive={pageNumber === currentPage} className={`cursor-pointer rounded-lg ${pageNumber === currentPage ? 'bg-green-600 hover:bg-green-700 text-white border-0' : 'hover:bg-green-50 hover:text-green-700 border-0'}`} onClick={(e) => { e.preventDefault(); setCurrentPage(pageNumber); }}>
                        {pageNumber}
                      </PaginationLink>
                    </PaginationItem>
                  );
                ...
                {totalPages > 5 && (
                  <>
                    ...
                    <PaginationItem><PaginationLink href='#' isActive={totalPages === currentPage} className="cursor-pointer rounded-lg hover:bg-green-50" onClick={(e) => { e.preventDefault(); setCurrentPage(totalPages); }}>{totalPages}</PaginationLink></PaginationItem>
                  </>
                )}
                <PaginationItem>
                  <PaginationNext href='#' className="cursor-pointer hover:bg-green-50 hover:text-green-700 rounded-lg" onClick={(e) => { e.preventDefault(); setCurrentPage(Math.min(totalPages, currentPage + 1)); }} />
                </PaginationItem>
              </PaginationContent>
            </Pagination>
          )}
