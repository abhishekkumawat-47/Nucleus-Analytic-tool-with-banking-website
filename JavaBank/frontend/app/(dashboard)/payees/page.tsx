  ...
  <Card className="overflow-hidden border border-gray-100 shadow-sm">
    <CardHeader className="p-5 pb-3 flex flex-row items-start justify-between space-y-0 bg-white">
      <div className="flex items-center space-x-3 w-full">
        <Skeleton className="h-12 w-12 rounded-full ring-2 ring-green-50 bg-zinc-200" />
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Dialog open={isAddPayeeOpen} onOpenChange={setIsAddPayeeOpen}>
          <DialogTrigger asChild>
            <Button className="rounded-full bg-green-600 hover:bg-green-700 text-white shadow-md hover:shadow-lg transition-all cursor-pointer">
          ...
          <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto w-[95vw]">
            ...
            <div className="grid gap-5 py-4">
              <div className="space-y-2 relative">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-9 rounded-lg border-green-200 focus-visible:ring-green-500"
                  />
                </div>
                    ...
                    {payeeSearchResults.map((res, i) => (
                      ...
                      >
                        <p className="font-bold text-sm text-zinc-900 flex items-center justify-between">
                          {res.name}
                          <span className="text-[9px] bg-green-100 text-green-600 px-1.5 py-0.5 rounded uppercase tracking-tighter ml-2">
                             {res.bankName}
                          </span>
                        </p>
              ...
              <div className="space-y-2">
                ...
                />
                {PayeeName ? (
                  <p className="ml-1 text-sm text-zinc-700 mt-2">Verified Banking Name: <span className="text-green-600 font-semibold">{PayeeName}</span></p>
                ) : null}
              </div>
              <div className="space-y-2">
            ...
            <DialogFooter className="flex flex-col sm:flex-row gap-3 sm:space-x-0 w-full sm:justify-between items-center">
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                <Button onClick={handleAddPayee} className="flex-1 bg-green-600 hover:bg-green-700 rounded-lg cursor-pointer">Add Payee</Button>
              </div>
            </DialogFooter>
          </DialogContent>
        </Dialog>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredPayees.length === 0 ? (
              <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-16 bg-gray-100 rounded-xl border border-dashed border-gray-200">
                <div className="h-16 w-16 bg-green-50 rounded-full flex flex-col items-center justify-center mx-auto mb-4">
              ...
              filteredPayees.map((payee) => (
                <Card key={payee.payeeAccNo} className="overflow-hidden border border-gray-100 hover:border-green-200 transition-all duration-300 hover:shadow-md group">
                  <CardHeader className="p-5 pb-3 flex flex-row items-start justify-between space-y-0 bg-white">
                    <div className="flex items-center space-x-3">
                      <Avatar className="h-12 w-12 border-2 border-white shadow-sm ring-2 ring-green-50">
                        <AvatarFallback
                          className={`
                            ...
                            ${payee.payeeType.toUpperCase() === "FINANCIAL" || payee.payeeType.toUpperCase() === "SHOPPING" ? "bg-gradient-to-br from-green-400 to-green-600" : ""}
                            ...
                            ${!["`OTHERS`", "GENERAL", "HOUSING", "FINANCIAL", "SHOPPING", "FOOD", "ENTERTAINMENT"].includes(payee.payeeType.toUpperCase()) ? "bg-gradient-to-br from-green-500 to-emerald-500" : ""}
                          `}
                        >
                      ...
                      <div className="overflow-hidden">
                        <CardTitle className="text-lg font-semibold truncate group-hover:text-green-700 transition-colors">{payee.name}</CardTitle>
                        <CardDescription className="text-sm font-mono mt-0.5">•••• {payee.payeeAccNo.slice(-4)}</CardDescription>
                      </div>
                    </div>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full cursor-pointer hover:bg-green-50">
                      ...
                      <DropdownMenuContent align="end" className="rounded-xl shadow-lg border-gray-100">
                        ...
                        <DropdownMenuItem
                          className="text-red-600 focus:bg-rose-50 focus:text-rose-700 cursor-pointer"
                  ...
                  <CardContent className="p-5 pt-3 bg-white">
                    <div className="grid grid-cols-1 gap-3 text-sm mb-4 p-3 bg-gray-100 rounded-lg border border-gray-100">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-7 w-7 rounded-lg hover:bg-green-100 text-muted-foreground hover:text-green-600"
                    ...
                    <Button
                      className="w-full rounded-xl bg-green-50 text-green-700 hover:bg-green-600 hover:text-white transition-all shadow-none group-hover:shadow-sm cursor-pointer"
                      onClick={() => {
      ...
      <Dialog open={isEditPayeeOpen} onOpenChange={setIsEditPayeeOpen}>
        <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto w-[95vw]">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => handleEditPayee(editPayee)} className="bg-green-600 hover:bg-green-700 rounded-lg shadow-md cursor-pointer">
              Save Changes
            </Button>
          </DialogFooter>
        </DialogContent>
