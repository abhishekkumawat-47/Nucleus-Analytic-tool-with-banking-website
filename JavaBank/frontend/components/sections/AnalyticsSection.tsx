...
const AnalyticsSection: React.FC<AnalyticsProps> = ({ accounts = [], transactions = [] }) => {
  ...
  const summaryCards = [
    { title: 'Total Balance', value: `₹${totalBalance.toLocaleString('en-IN')}`, icon: <CreditCard className="h-5 w-5" />, color: 'bg-green-100 text-green-600' },
    { title: 'Income', value: `₹${income.toLocaleString('en-IN')}`, icon: <TrendingUp className="h-5 w-5" />, color: 'bg-emerald-100 text-emerald-600' },
    { title: 'Expenses', value: `₹${expenses.toLocaleString('en-IN')}`, icon: <TrendingDown className="h-5 w-5" />, color: 'bg-rose-100 text-red-600' },
    { title: 'Transactions', value: transactions.length.toLocaleString('en-IN'), icon: <Activity className="h-5 w-5" />, color: 'bg-green-100 text-green-600' },
  ];
    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {summaryCards.map((card, index) => (
          <div key={index} className="flex flex-col items-center p-5 bg-white border border-green-100 rounded-xl shadow-sm hover:shadow-md transition-all cursor-default">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-5 border border-green-100 rounded-xl bg-white shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={monthlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="amount" fill="#16A34A" radius={[4, 4, 0, 0]} barSize={32} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        {/* Category Breakdown Pie Chart */}
        <div className="p-5 border border-green-100 rounded-xl bg-white shadow-sm flex flex-col">
