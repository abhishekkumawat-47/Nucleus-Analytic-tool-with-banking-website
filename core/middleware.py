...
def require_tenant_access(requested_tenant_id: str):
    """
    Dependency (or helper) to ensure the requested tenant matches the local instance
    when running in ON_PREM mode.
    """
    if settings.is_on_prem:
        if requested_tenant_id != settings.TENANT_ID:
            logger.warning(f"Blocked access to tenant '{requested_tenant_id}'. Active local tenant is '{settings.TENANT_ID}'.")
            raise HTTPException(
                status_code=403, 
                detail="Forbidden: Cross-tenant data access is blocked in ON_PREM mode."
            )
